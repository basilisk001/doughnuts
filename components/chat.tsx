...
export function Chat({ modelId = DEFAULT_MODEL }: { modelId: string }) {
  ...
  const handleNewChat = () => { stop(); setMessages([]); setInput(""); };

  return (
    <div className="h-screen w-screen relative overflow-hidden bg-black dark:bg-black text-white">
      {/* === Stars & comet layers === */}
      ...
      <div className="relative z-10 flex flex-col h-full">
        {/* Top Controls */}
        <div className="absolute top-3 left-3 md:top-4 md:left-4 z-20 flex gap-2 animate-fade-in">
          <Button
        ...
        {!hasMessages && (
          <div className="flex-1 flex flex-col items-center justify-center px-4 md:px-8 animate-fade-in">
            <div className="w-full max-w-2xl text-center space-y-8 md:space-y-12">
              <h1 className="text-3xl md:text-6xl font-light tracking-tight text-white animate-slide-up">
                  ...
                  <div className="flex items-center gap-2 md:gap-3 p-3 md:p-4 rounded-2xl glass-effect shadow-border-medium transition-all duration-200 ease-out">
                    ...
                    <div className="flex flex-1 items-center">
                      <Input
                        ...
                        value={input}
                        autoFocus
                        className="flex-1 border-0 bg-transparent focus-visible:ring-0 text-base placeholder:text-white/60"
                        onKeyDown={(e) => {
                      ...
                      <Button
                        ...
                        size="icon"
                        variant="ghost"
                        className="h-9 w-9 rounded-xl hover:bg-white/20"
                        disabled={!input.trim()}
                      >
                ...
                {messages.map((m) => (
                  <div
                    ...
                    className={cn(
                      ...
                      m.role === "assistant" &&
                        "max-w-[95%] md:max-w-[85%] text-white/90 leading-relaxed text-sm md:text-base"
                    )}
                  >
                    {m.parts.map((part, i) => {
              ...
              <div className="flex items-center gap-3 p-4 rounded-2xl glass-effect shadow-border-medium transition-all duration-200 ease-out">
                ...
                <div className="flex flex-1 items-center">
                  <Input
                    ...
                    onChange={(e) => setInput(e.target.value)}
                    value={input}
                    className="flex-1 border-0 bg-transparent focus-visible:ring-0 text-base placeholder:text-white/60 font-medium"
                    onKeyDown={(e) => {
                      if (e.metaKey && e.key === "Enter") {
                        sendMessage({ text: input }, { body: { modelId: currentModelId } });
                        setInput("");
                  ...
                  <Button
                    ...
                    size="icon"
                    variant="ghost"
                    className="h-9 w-9 rounded-xl hover:bg-white/20 hover:scale-110 transition-all duration-150 ease disabled:opacity-50 disabled:hover:scale-100"
                    disabled={!input.trim()}
                  >
                    <SendIcon className="h-4 w-4" />
                  </Button>
