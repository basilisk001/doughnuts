      ...
      {!hasMessages && (
        <div className="flex-1 flex flex-col items-center justify-center px-4 md:px-8 animate-fade-in">
          <div className="w-full max-w-2xl text-center space-y-8 md:space-y-12">
            <h1 className="text-3xl md:text-6xl font-light tracking-tight animate-slide-up">
              <span className="font-mono font-semibold tracking-tight bg-background/0 text-white px-4 py-3 rounded-2xl shadow-border-medium glass-effect">
                AI GATEWAY
              </span>
            </h1>
            ...
            <div className="w-full animate-slide-up" style={{ animationDelay: "100ms" }}>
              ...
              >
                <div className="flex items-center gap-2 md:gap-3 p-3 md:p-4 rounded-2xl glass-effect shadow-border-medium transition-all duration-200 ease-out">
                  <ModelSelectorHandler modelId={modelId} onModelIdChange={setCurrentModelId} />
                  <div className="flex flex-1 items-center">
                    ...
                    />
                    <Button type="submit" size="icon" variant="ghost" className="h-9 w-9 rounded-xl hover:bg-muted/50" disabled={!input.trim()}>
      ...
      {hasMessages && (
        <div className="w-full max-w-4xl mx-auto px-4 md:px-8 pb-6 md:pb-8">
          <form
            ...
            className="flex items-center gap-3 p-4 rounded-2xl glass-effect shadow-border-medium transition-all duration-200 ease-out"
          >
            <ModelSelectorHandler modelId={modelId} onModelIdChange={setCurrentModelId} />
            <div className="flex flex-1 items-center">
              <Input
      ...
      {error && (
        <div className="max-w-4xl mx-auto w-full px-4 md:px-8 pb-4 animate-slide-down">
          <Alert variant="destructive">
            ...
            <AlertDescription>An error occurred while generating the response.</AlertDescription>
            <Button variant="outline" size="sm" className="ml-auto transition-all duration-150 ease-out hover:scale-105" onClick={() => regenerate()}>
              Retry
            </Button>
          </Alert>
        </div>
      ...
      <footer className="pb-8 text-center animate-fade-in" style={{ animationDelay: "200ms" }}>
        <p className="text-xs md:text-sm text-muted-foreground">
          ...
          <Button variant="link" asChild className="p-0 h-auto text-xs md:text-sm font-normal">
            <a href="https://vercel.com/d?to=%2F%5Bteam%5D%2F%7E%2Fai%2Fmodel-list&title=" target="_blank" rel="noopener noreferrer">
              model library
            </a>
          </Button>{" "}
          for the full set.
